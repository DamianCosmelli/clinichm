...
import { obtenerPacientesSinVisitaEnUltimosMeses } from '../../../services/pacientesService'; // Método para obtener pacientes
...
const SinVisita: React.FC = () => {
  const [pacientes, setPacientes] = useState<Paciente[]>([]);
  const [rows, setRows] = useState<{ Documento: string; Paciente: string; 'Última visita': string; Contacto: string }[]>([]);
  const [mensajesCopiados, setMensajesCopiados] = useState<{ [key: string]: boolean }>({});
  const [itemsPerPage, setItemsPerPage] = useState(20);
  const [currentPage, setCurrentPage] = useState(1);
  const [filtro, setFiltro] = useState({ buscar: '', meses: 6 }); // Filtro con meses y búsqueda
  ...
  useEffect(() => {
    const cargarPacientes = async () => {
      try {
        setLoading(true);
        const data = await obtenerPacientesSinVisitaEnUltimosMeses(filtro.meses);
        const datos = data
          .filter((paciente) => {
            const fechaHoraIngreso = paciente.horaIngreso ? new Date(paciente.horaIngreso) : null;
            const fechaLimite = new Date();
            fechaLimite.setMonth(fechaLimite.getMonth() - filtro.meses);
            return fechaHoraIngreso && fechaHoraIngreso >= fechaLimite; // Filtrar por última visita en los últimos meses
          })
          .map((paciente) => ({
            Documento: paciente.dni,
            Paciente: `${paciente.nombre} ${paciente.apellido}`,
            'Última visita': paciente.horaIngreso ? paciente.horaIngreso.split('T')[0] : 'N/A',
  ...
  useEffect(() => {
    ...
    filtrarPacientes();
  }, [filtro.buscar, pacientes, itemsPerPage, currentPage]);
  ...
  return (
    <div>
      <span className="text-subtitulo mb-4 font-poppins">Sin visita en los últimos meses</span>
      {/* Filtros */}
      <div className="flex items-center justify-center gap-4 mt-4">
        {/* Filtro de meses */}
        <div className="flex items-center w-60 h-10 px-3 border border-gray-400 rounded bg-gray-100">
          <label className="mr-2 text-sm font-poppins text-black">Meses: </label> {/* Label fuera del input */}
          <input
            type="number"
            ...
            value={filtro.meses}
            onChange={(e) => {
              const value = Math.min(parseInt(e.target.value, 10) || 1, 6); // Limitar a un máximo de 6
      ...
      <div className="mt-6">
        <div className={`overflow-y-auto ${rows.length > itemsPerPage ? 'max-h-[420px]' : ''}`}>
          <TableListados
            headers={['Documento', 'Paciente', 'Última visita', 'Contacto']}
            rows={rows.map((row) => ({
              ...row,
              Documento: (
