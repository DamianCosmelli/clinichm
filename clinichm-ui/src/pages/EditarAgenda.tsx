...
import { Link, useLocation } from "react-router-dom"; // Importar Link para navegación
...
import FormularioEditarAgenda from "../components/AgendaMedica/FormularioEditarAgenda";
import Loading from "./common/Loading";
import useCargarAgenda from "../hooks/useCargarAgenda";

const EditarAgenda: React.FC = () => {
  const location = useLocation();
  const { id } = location.state || {}; // Obtener el id del estado de navegación

  const agenda = useCargarAgenda(id); // Pasar el id al hook
    ...
    <div className="w-full h-screen bg-fondo-contenedor relative overflow-hidden">
      <div className="flex flex-col gap-2 px-4 py-1 -mt-2 -ml-4">
        ...
        <div className="flex items-center gap-1">
          <Link to="/agenda" className="text-navegacion">
            Agenda
          </Link>
          <img
            ...
            className="w-4 h-4"
          />
          <span className="text-navegacion">Editar Agenda</span>
        </div>
        {/* Subtítulo y mensaje de éxito */}
        <div className="relative flex justify-between items-center">
          <span className="text-subtitulo mb-4 -mt-4">Editar Agenda</span>{" "}
